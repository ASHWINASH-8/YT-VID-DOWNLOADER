...
def get_available_formats(url: str) -> None:
    ...
    try:
        with YoutubeDL(ydl_opts) as ydl:
            info = ydl.extract_info(url, download=False)
            if info:
                print("\n📋 Available formats for video:")
                print("=" * 80)
                print(f"{'Format Code':<12} {'Extension':<10} {'Resolution':<12} {'Note':<15} {'Filesize':<12}")
                print("-" * 80)
                ...
                for f in formats:
                    ...
                    # Note for audio-only or video-only
                    note = []
                    if f.get('acodec', 'none') == 'none':
                        note.append("Video only")
                    elif f.get('vcodec', 'none') == 'none':
                        note.append("Audio only")
                    
                    # Add quality note
                    if 'format_note' in f and f['format_note']:
                        note.append(f['format_note'])
                        
                    note_str = ", ".join(note)
                    
                    print(f"{format_id:<12} {ext:<10} {resolution:<12} {note_str:<15} {filesize:<12}")
    except Exception as e:
        print(f"Error listing formats: {str(e)}")


def download_single_video(url: str, output_path: str, thread_id: int = 0, audio_only: bool = False, resolution: str = None) -> dict:
    """
    ...
    Args:
        ...
        resolution (str, optional): Specific video resolution to download (e.g., '720p', '1080p')
    ...
    if audio_only:
        ...
        # Configure for video downloads with specific resolution if provided
        if resolution:
            height = resolution.replace('p', '')  # Convert '720p' to '720'
            try:
                height = int(height)
                format_selector = f'bestvideo[height={height}]/bestvideo[height<={height}]/best'
                print(f"🎬 [Thread {thread_id}] Targeting {resolution} resolution, video only...")
            except ValueError:
                # If resolution isn't a valid number, use default behavior
                format_selector = 'bestvideo[height<=1080]/best'
                print(f"⚠️ [Thread {thread_id}] Invalid resolution '{resolution}', using default (video only)...")
        else:
            # Default format selector (up to 1080p)
            format_selector = 'bestvideo[height<=1080]/best'
        file_extension = 'mp4'
        postprocessors = [{
    ...
    # Video downloads don't need merge_output_format since we're not combining audio+video
    # But we do need to specify the format
    if not audio_only:
...
def download_youtube_content(urls: List[str], output_path: Optional[str] = None,
    ...
    if audio_only:
        print(f"🎧 Format: MP3 Audio Only")
    else:
        resolution_str = f" ({resolution})" if resolution else ""
        print(f"� Format: MP4 Video Only{resolution_str}")
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        future_to_url = {
            executor.submit(download_single_video, url, output_path, i+1, audio_only, resolution): url
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == '--list-formats':
        ...
        if format_choice == '2':
            ...
            # Ask for specific resolution when video is selected
            resolution_choice = input(
                "\nSelect video resolution (or press Enter for best quality):\n"
                "  1. 360p (Low quality, smaller size)\n"
                "  2. 480p (Standard definition)\n"
                "  3. 720p (HD)\n"
                "  4. 1080p (Full HD, default)\n"
                "  5. 1440p (2K)\n"
                "  6. 2160p (4K)\n"
                "Enter choice (1-6, default=4): ").strip()
            ...
            if resolution_choice in resolution_map:
                ...
                print("⚠️ Invalid choice. Using best quality (up to 1080p)")
            else:
                print("📌 Using best quality (up to 1080p)")
        ...
        if output_dir:
            download_youtube_content(
                urls, output_dir, max_workers=max_workers, audio_only=audio_only, resolution=resolution)
        else:
            download_youtube_content(
                urls, max_workers=max_workers, audio_only=audio_only, resolution=resolution)